...
climParam.numSampTS = 100; % updated 9/28/2021 100;
...
costParam.PercFlex = x(9); 

% Expansion cost of flexible dam
costParam.PercFlexExp = x(10);
...
if runParam.calcTmat
    ...
    %load('T_Temp_Precip_RCP85B_KW', 'T_Precip') % updated from Jenny- RCP85B
    %T_Precip; 
            ...
            for index_s_p = index_s_p_thisPeriod
                ...
                % Call CLIRUN streamflow simulator
                 runoff_temp{index_s_p} = ...
                     ...
                     repmat(P_ts{index_s_p,t}, [1 N]), N*(runParam.steplen));
...
if runParam.calcShortage
    ...
    if runParam.optReservoir % DDP calculated shortage costs
        ...
        for i=1:length(storage)
            s_state = string(storage(i));
            s_state_filename = strcat('sdp_nonadaptive_shortage_cost_s',s_state,'.mat'); %Nov 2021 data
            %s_state_filename = strcat('sdp_nonadaptive_shortage_cost_domagCost231_s',s_state,'.mat'); % Oct 2021 data
            shortageCostDir = load(s_state_filename,'shortageCost');
            shortageCost_s_state = shortageCostDir.shortageCost(:,:); %18:49
            shortageCost(:,:,i,1) = shortageCost_s_state;
            % load shortage volumes (unmet dom and ag) and compile into
...
shortageCost = shortageCost.*costParam.cPrime; % JS moved this line for working w/ sdp
...
VolShort = NaN(M_T_abs, M_P_abs, M_C, N+1);
...
VolShort(:,:,:,N+1) = zeros(M_T_abs, M_P_abs, M_C, 1);
        ...
        for index_s_p = index_s_p_thisPeriod
            ...
            for index_s_c = 1:M_C
                ...
                for index_a = 1:num_a_exp
                    ...
                    expShortage = sum(T(indexNonZeroT) .*nextVolSh(indexNonZeroT));
                    for i = 2:4
                        expV = sum(expV);
                        expVs = sum(expVs);
                        expVd = sum(expVd);
                        expShortage = sum(expShortage);
                    end
                    
                   % get shortage in current state
                   shortage = shortageVol(index_s_t, index_s_p, index_s_c);
                   ...
                   % Check if best decision
                    ...
                    if checkV < bestV
                        ...
                        bestVd = dCostD + expVd;
                        bestVolSh = shortage + expShortage;
                    end
            ...
            VolShort(index_s_t, index_s_p,index_s_c, t) = bestVolSh;
            end
        end
    end
end
