...
mkdir('Nov02optimal_dam_design_percFlex')

percFlex = [0.05, 0.075, 0.1]; % range of discount rates to consider

for p = 1:length(percFlex)
    pFlex = percFlex(p); % new discount rate to use for simulations
...
% 1. Find the optimal flexible design dam size
        ...
        for h=1 % flex expansion increments
            for g=1:7 % number of flex expansion(7-2*h)
                ...
                if (j*1.6) < (j+h*10*g) % if false, then this combination is ok
                    ...
                    x(6) = pFlex; %costParam.PercFlex: initial upfront capital cost increase flex design
    ...
    if z == 2
        file_name = strcat('Nov02optimal_dam_design_percFlex/BestFlex_adaptive_cp6e6_g7_percFlex',string(pFlex*100),'_percExp15_disc3');
        save(file_name, 'bestAct_flex','bestVal_flex','allV_flex','allX_flex');
    elseif z == 1
        file_name = strcat('Nov02optimal_dam_design_percFlex/BestFlex_nonadaptive_cp6e6_g7_percFlex',string(pFlex*100),'_percExp15_disc3');
        save(file_name, 'bestAct_flex','bestVal_flex','allV_flex','allX_flex');
    end
...
% 2. Find the optimal static dam size
    ...
    if z == 2
        file_name = strcat('Nov02optimal_dam_design_percFlex/BestStatic_adaptive_cp6e6_g7_percFlex',string(pFlex*100),'_percExp15_disc3');
        save(file_name, 'bestAct_static','bestVal_static','allV_static','allX_static');
    elseif z == 1
        file_name = strcat('Nov02optimal_dam_design_percFlex/BestStatic_nonadaptive_cp6e6_g7_percFlex',string(pFlex*100),'_percExp15_disc3');
        save(file_name, 'bestAct_static','bestVal_static','allV_static','allX_static');
    end
...
% 3. Find the optimal flexible planning dam size
    ...
    if z == 2
        file_name = strcat('Nov02optimal_dam_design_percFlex/BestPlan_adaptive_cp6e6_g7_percFlex',string(pFlex*100),'_percExp15_disc3');
        save(file_name, 'bestAct_plan','bestVal_plan','allV_plan','allX_plan');
    elseif z == 1
        file_name = strcat('Nov02optimal_dam_design_percFlex/BestPlan_nonadaptive_cp6e6_g7_percFlex',string(pFlex*100),'_percExp15_disc3');
        save(file_name, 'bestAct_plan','bestVal_plan','allV_plan','allX_plan');
    end
...
% 4. Run the SDP and forward simulation with the optimal dam sizes
    ...
    run('multiflex_sdp_climate_StaticFlex_DetT_Nov2021');
    if z == 2
        file_name = strcat('Nov02optimal_dam_design_percFlex/BestFlexStaticPlan_adaptive_cp6e6_g7_percFlex',string(pFlex*100),'_percExp15_disc3');
        save(file_name, 'bestAct', 'V', 'X','action', 'totalCostTime','damCostTime','C_state','P_state')
    elseif z == 1
        file_name = strcat('Nov02optimal_dam_design_percFlex/BestFlexStaticPlan_nonadaptive_cp6e6_g7_percFlex',string(pFlex*100),'_percExp15_disc3');
        save(file_name, 'bestAct', 'V', 'X', 'action', 'totalCostTime','damCostTime','C_state','P_state')
    end
