...
% 1. Find the optimal flexible design dam size
    ...
    for j=30:10:140 %200 %140 % initial unexpanded dam sizes
        for h=1 % flex expansion increments
            for g=1:7 % number of flex expansion(7-2*h)
                ...
                elseif (j+h*10*g) > 210 % place max on possible dam size
                    disp('Flex design size not feasible');
                else
                    ...
                    x(15) = 0.03; %0.03; %costParam.discountrate
    ...
    if z == 2
        file_name = strcat('Jan28optimal_dam_design_comb/BestFlex_adaptive_cp6e6_g7_percFlex75_percExp15_disc3');
        save(file_name, 'bestAct_flex','bestVal_flex','allV_flex','allX_flex','allVs_flex','allVd_flex');
    elseif z == 1
        file_name = strcat('Jan28optimal_dam_design_comb/BestFlex_nonadaptive_cp6e6_g7_percFlex75_percExp15_disc3');
        save(file_name, 'bestAct_flex','bestVal_flex','allV_flex','allX_flex','allVs_flex','allVd_flex');
    end
...
% 2. Find the optimal static dam size
    ...
    if z == 2
        file_name = strcat('Jan28optimal_dam_design_comb/BestStatic_adaptive_cp6e6_g7_percFlex75_percExp15_disc3');
        save(file_name, 'bestAct_static','bestVal_static','allV_static','allX_static','allVs_static','allVd_static');
    elseif z == 1
        file_name = strcat('Jan28optimal_dam_design_comb/BestStatic_nonadaptive_cp6e6_g7_percFlex75_percExp15_disc3');
        save(file_name, 'bestAct_static','bestVal_static','allV_static','allX_static','allVs_static','allVd_static');
    end
...
% 3. Find the optimal flexible planning dam size
    ...
    if z == 2
        file_name = strcat('Jan28optimal_dam_design_comb/BestPlan_adaptive_cp6e6_g7_percFlex75_percExp15_disc3');
        save(file_name, 'bestAct_plan','bestVal_plan','allV_plan','allX_plan','allVs_plan','allVd_plan');
    elseif z == 1
        file_name = strcat('Jan28optimal_dam_design_comb/BestPlan_nonadaptive_cp6e6_g7_percFlex75_percExp15_disc3');
        save(file_name, 'bestAct_plan','bestVal_plan','allV_plan','allX_plan','allVs_plan','allVd_plan');
    end
...
% 4. Run the SDP and forward simulation with the optimal dam sizes
    ...
    run('multiflex_sdp_climate_StaticFlex_DetT_Nov2021');
    if z == 2
        file_name = strcat('Jan28optimal_dam_design_comb/BestFlexStaticPlan_adaptive_cp6e6_g7_percFlex75_percExp15_disc3');
        save(file_name, 'bestAct', 'V', 'X','action', 'totalCostTime','damCostTime','C_state','P_state','Vs','Vd')
    elseif z == 1
        file_name = strcat('Jan28optimal_dam_design_comb/BestFlexStaticPlan_nonadaptive_cp6e6_g7_percFlex75_percExp15_disc3');
        save(file_name, 'bestAct', 'V', 'X', 'action', 'totalCostTime','damCostTime','C_state','P_state','Vs','Vd')
    end
