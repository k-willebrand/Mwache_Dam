...
% February 2022 - FLEX DESIGN AND PLAN ALWAYS CAN EXPAND UP TO 50% CAP.
% For one set of transition matrices, do the following:
%   1. Find the optimal flexible design dam size 
%      - SET MAX EXPANSION ALWAYS TO 50% OF INITIAL CAPACITY
%   INITIAL CAPACITY
...
mkdir('Feb7optimal_dam_design_comb')

cps = [1.25e-6, 3.75e-6];
discounts = [0 3 6]; % percent (%)

for dsc = 1:2 %:length(discounts)
...
% 1. Find the optimal flexible design dam size
        ...
        for h=1 % flex expansion increments
            for g=1:7 % number of flex expansion(7-2*h)
                ...
                if (j+h*10*g) ~= ceil(1.5*j/10)*10 % if false, then this combination is ok
                    disp('Flex design max expansion not 50% of initial capacity');
                else
                    ...
                    x(7) = 0.15; %costParam.PercFlexExp: expansion cost of flexible dam  flex design (0)
                    ...
                    x(12) = 2.5; %costParam.PercPlanExp: expansion cost of flexibly planned dam (0.5)
                    ...
                    x(15) = discounts(dsc)/100; %0.03; %costParam.discountrate
                    ...
                    if val < bestVal_flex
                        ...
                        allVs_flex = Vs;
                        allVd_flex = Vd;
                        allVs_flex_nondisc = Vs_nondisc;
                        allVd_flex_nondisc = Vd_nondisc;
    ...
    if z == 2
        file_name = strcat('Feb7optimal_dam_design_comb/BestFlex_adaptive_cp', regexprep(strrep(string(c_prime), '.', ''), {'-0'}, {''}),'_g7_percFlex75_percExp15_disc',string(discounts(dsc)));
        save(file_name, 'bestAct_flex','bestVal_flex','allV_flex','allX_flex','allVs_flex','allVd_flex', 'allVs_flex_nondisc','allVd_flex_nondisc');
    elseif z == 1
        file_name = strcat('Feb7optimal_dam_design_comb/BestFlex_nonadaptive_cp', regexprep(strrep(string(c_prime), '.', ''), {'-0'}, {''}),'_g7_percFlex75_percExp15_disc',string(discounts(dsc)));
        save(file_name, 'bestAct_flex','bestVal_flex','allV_flex','allX_flex','allVs_flex','allVd_flex', 'allVs_flex_nondisc', 'allVd_flex_nondisc');
    end
...
% 2. Find the optimal static dam size
    ...
    for j=30:10:150 %210 %150
        ...
        if val < bestVal_static
            ...
            allVs_static = Vs;
            allVd_static = Vd;
            allVs_static_nondisc = Vs_nondisc;
            allVd_static_nondisc = Vd_nondisc;
    ...
    if z == 2
        file_name = strcat('Feb7optimal_dam_design_comb/BestStatic_adaptive_cp', regexprep(strrep(string(c_prime), '.', ''), {'-0'}, {''}),'_g7_percFlex75_percExp15_disc',string(discounts(dsc)));
        save(file_name, 'bestAct_static','bestVal_static','allV_static','allX_static','allVs_static','allVd_static','allVs_static_nondisc','allVd_static_nondisc');
    elseif z == 1
        file_name = strcat('Feb7optimal_dam_design_comb/BestStatic_nonadaptive_cp', regexprep(strrep(string(c_prime), '.', ''), {'-0'}, {''}),'_g7_percFlex75_percExp15_disc',string(discounts(dsc)));
        save(file_name, 'bestAct_static','bestVal_static','allV_static','allX_static','allVs_static','allVd_static','allVs_static_nondisc','allVd_static_nondisc');
    end
...
% 3. Find the optimal flexible planning dam size
    ...
    for j=30:10:140 %200 %140
        ...
        for h=1 % flex expansion increments
            for g=1:7 % number of flex expansion(7-2*h)
                ...
                %if (j*1.6) < (j+h*10*g) % if false, then this combination is ok
                if (j+h*10*g) ~= ceil(1.5*j/10)*10 % if false, then this combination is ok
                    disp('Flex plan max expansion not 50% of initial capacity');
                else
                    ...
                    if val < bestVal_plan
                        ...
                        allVs_plan = Vs;
                        allVd_plan = Vd;
                        allVs_plan_nondisc = Vs_nondisc;
                        allVd_plan_nondisc = Vd_nondisc;
    ...
    if z == 2
        file_name = strcat('Feb7optimal_dam_design_comb/BestPlan_adaptive_cp', regexprep(strrep(string(c_prime), '.', ''), {'-0'}, {''}),'_g7_percFlex75_percExp15_disc',string(discounts(dsc)));
        save(file_name, 'bestAct_plan','bestVal_plan','allV_plan','allX_plan','allVs_plan','allVd_plan','allVs_plan_nondisc','allVd_plan_nondisc');
    elseif z == 1
        file_name = strcat('Feb7optimal_dam_design_comb/BestPlan_nonadaptive_cp', regexprep(strrep(string(c_prime), '.', ''), {'-0'}, {''}),'_g7_percFlex75_percExp15_disc',string(discounts(dsc)));
        save(file_name, 'bestAct_plan','bestVal_plan','allV_plan','allX_plan','allVs_plan','allVd_plan','allVs_plan_nondisc','allVd_plan_nondisc');
    end
...
% 4. Run the SDP and forward simulation with the optimal dam sizes
    ...
    run('multiflex_sdp_climate_StaticFlex_DetT_Nov2021_cprime');
    if z == 2
        file_name = strcat('Feb7optimal_dam_design_comb/BestFlexStaticPlan_adaptive_cp', regexprep(strrep(string(c_prime), '.', ''), {'-0'}, {''}),'_g7_percFlex75_percExp15_disc',string(discounts(dsc)));
        save(file_name, 'bestAct', 'V', 'X','action', 'totalCostTime','damCostTime','C_state','P_state','Vs','Vd','Vs_nondisc','Vd_nondisc')
    elseif z == 1
        file_name = strcat('Feb7optimal_dam_design_comb/BestFlexStaticPlan_nonadaptive_cp', regexprep(strrep(string(c_prime), '.', ''), {'-0'}, {''}),'_g7_percFlex75_percExp15_disc',string(discounts(dsc)));
        save(file_name, 'bestAct', 'V', 'X', 'action', 'totalCostTime','damCostTime','C_state','P_state','Vs','Vd','Vs_nondisc','Vd_nondisc')
    end
