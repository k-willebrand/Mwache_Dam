function [T_ts, P_ts, model_rand, year_rand_start] = mean2TPtimeseriesMJL_2(timestep, steplen, numsamp)
% updated September 2021 by JBS to take in steplen = 110 years and then to
% output 100 years of data
...
%daterange = (timestep-1)*steplen*12 + startdate: (timestep)*steplen*12 + startdate - 1; % SSD: Second period starts in 2010
daterange = (timestep-1)*(steplen)*12 + startdate: (timestep)*(steplen)*12 + startdate - 1;
steplenT = 22;
daterangeT = (timestep-1)*(steplenT)*12 + startdate: (timestep)*(steplenT)*12 + startdate - 1;
%% Load Pij and Tij

load('Mombasa_TandP.mat', 'Tij', 'Pij')
%load('Mombasa_TandP-45-85.mat', 'Pij_85', 'Tij_85');
%Pij = Pij_85;
%Tij = Tij_85;

% Create standardized anomalies relative to decadal average across all GCMs for the current daterange
Tmean_gcm = mean(Tij(daterangeT,:),1);
Pmean_gcm = mean(Pij(daterange,:),1);
T_anoms = Tij(daterangeT,:) - repmat(Tmean_gcm, length(daterangeT),1);
P_anoms = Pij(daterange,:) - repmat(Pmean_gcm, length(daterange),1);

% standardize the monthly anomalies for each gcm- precip
for month = 1:12
    monthrange = month:12:1320; %240
...
% standardize the monthly anomalies for each gcm- precip
for month = 1:12
    monthrange = month:12:steplenT*12;
    Tmean_month(monthrange,:) = repmat(mean(T_anoms(monthrange,:),1),steplenT,1);
    Tstd_month(monthrange,:) = repmat(std(T_anoms(monthrange,:),1),steplenT,1);
end   

T_norm = (T_anoms - Tmean_month)./Tstd_month; % 'deseasonalized' values of T
T_norm = repmat(T_norm, 5, 1);
...
T_ts = T_tsp.*repmat(Tstd_month(:,model_rand)', 1, 5) + repmat(Tmean_month(:,model_rand)', 1, 5);%+Tmean*ones(size(T_tsp));
P_ts = P_tsp.*Pstd_month(:,model_rand)' + Pmean_month(:,model_rand)';%+Pmean*ones(size(P_tsp));
% take 100 years of data out of 110
T_ts = T_ts(:,1:(steplen-10)*12);
P_ts = P_ts(:,1:(steplen-10)*12);
T_ts = double(T_ts);
P_ts = double(P_ts);
