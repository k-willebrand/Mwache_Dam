...
% This version of the script is not parallized. 
...
% non-adaptive operating policy. To run this script, update the storage values (can be an
% array), date, and filepath. Set the values for runParam, costParam, 
% climParam, and sys_param fields accordingly.

% NOTE: For climate-adaptive operations, set runParam.adaptiveOps = true 
% else set runParam.adaptiveOps = false.
...
% Define reservoir capacities (can be an array of capacities) and dead storage
storage_vals = [85]; % set reservoir capacity (MCM)
dead_storage = 20; % MCM
...
runParam.domDemand = 186000; % m3/d
...
% preallocate shortage cost file
shortageCost = NaN(num_T_states,num_P_states);
unmet =  NaN(num_T_states,num_P_states);
unmet_ag =  NaN(num_T_states,num_P_states);
unmet_ag2 = NaN(num_T_states,num_P_states);
unmet_dom =  NaN(num_T_states,num_P_states);
unmet_dom2 =  NaN(num_T_states,num_P_states);
...
for ss=1:length(storage_vals)
    ...
    for s_T=1:num_T_states
        ...
        for s_P = 1:num_P_states
            
            P_state = s_P;
            disp(strcat('s_T:   ',string(s_T),' s_P:   ',string(s_P)));
            ...
            shortageCost(T_state,P_state) = mean(sum(J)); % $
            unmet(T_state,P_state) = mean(sum(dmd_unmet)); % CM
            unmet_ag(T_state,P_state) = mean(sum(dmd_unmet_ag)); % CM
            unmet_ag2(T_state,P_state) = mean(sum(dmd_unmet_ag.^2)); % CM
            unmet_dom(T_state,P_state) = mean(sum(dmd_unmet_dom)); % CM
            unmet_dom2(T_state,P_state) = mean(sum(dmd_unmet_dom.^2)); % CM^2
        end
    end
    
    % save shortage cost outputs
    if runParam.saveOn
        if runParam.adaptiveOps
            filename = strcat('adaptive_shortage_cost_domCost1_RCP85_s',string(storage),'_',date,'.mat');
        else
            filename = strcat('nonadaptive_shortage_cost_domCost1_RCP85_s',string(storage),'_',date,'.mat');
        end
        save(filename, 'shortageCost', 'unmet', 'unmet_ag', 'unmet_ag2', 'unmet_dom', 'unmet_dom2');
    end
