...
if ~isempty(getenv('SLURM_JOB_ID'))
    ...
    folder = '/home/users/keaniw/Fletcher_2019_Learning_Climate/SDP_reservoir_ops/Oct17sdp_reservoir_ops_SteadyState';
else
    projpath = 'C:/Users/kcuw9/Documents/Mwache_Dam';
    jobid = 'na';
    folder = 'C:/Users/kcuw9/Documents/Mwache_Dam/SDP_reservoir_ops/Oct17sdp_reservoir_ops_SteadyState'; 
end
addpath(genpath(projpath))
%addpath('C:\Users\kcuw9\Documents\Mwache_Dam\SDP_reservoir_ops\sdp_reservoir_ops_SteadyState')
addpath('C:\Users\kcuw9\Documents\Mwache_Dam\SDP_reservoir_ops\Oct17sdp_reservoir_ops_SteadyState')

mkdir('Oct17post_process_sdp_reservoir_results')
...
if runParam.adaptiveOps
    cluster_files = dir(fullfile(folder,'Oct172021adaptive_domagCost231_SSTest_st*_sp*_s*.mat'));
else
    cluster_files = dir(fullfile(folder,'Oct172021nonadaptive_domagCost231_SSTest_st*_sp*_s*.mat'));
end
...
for i = 1:length(s_names)
    if runParam.adaptiveOps
        savename_shortageCost = strcat('Oct17post_process_sdp_reservoir_results/sdp_adaptive_shortage_cost_s', string(s_names(i)));
    else
        savename_shortageCost = strcat('Oct17post_process_sdp_reservoir_results/sdp_nonadaptive_shortage_cost_s', string(s_names(i)));
    end
