...
if ~isempty(getenv('SLURM_JOB_ID'))
    ...
    folder = '/home/users/keaniw/Fletcher_2019_Learning_Climate/SDP_reservoir_ops/Apr06sdp_reservoir_ops_SteadyState'
else
    projpath = 'C:/Users/kcuw9/Documents/Mwache_Dam';
    jobid = 'na';
    folder = 'C:/Users/kcuw9/Documents/Mwache_Dam/SDP_reservoir_ops/Apr06sdp_reservoir_ops_SteadyState';
end
...
addpath('/home/users/keaniw/Fletcher_2019_Learning_Climate/SDP_reservoir_ops/Apr06sdp_reservoir_ops_SteadyState')

%mkdir('Nov02post_process_sdp_reservoir_results')
mkdir('Apr06post_process_sdp_reservoir_results')
...
for opType = 1:2 % for adaptive and non-adaptive reservoir operations
    ...
    if runParam.adaptiveOps
        cluster_files = dir(fullfile(folder,'Apr062021adaptive_domagCost231_SSTest_st*_sp*_s*.mat'));
    else
        cluster_files = dir(fullfile(folder,'Apr062021nonadaptive_domagCost231_SSTest_st*_sp*_s*.mat'));
    end
    ...
    for i = 1:length(s_names)
        if runParam.adaptiveOps
            %savename_shortageCost = strcat('Apr06post_process_sdp_reservoir_results/sdp_adaptive_shortage_cost_s', string(s_names(i)));
            savename_storage = strcat('Apr06post_process_sdp_reservoir_results/sdp_adaptive_storage_s', string(s_names(i)));
        else
            %savename_shortageCost = strcat('Nov02post_process_sdp_reservoir_results/sdp_nonadaptive_shortage_cost_s', string(s_names(i)));
            savename_storage = strcat('Apr06post_process_sdp_reservoir_results/sdp_nonadaptive_storage_s', string(s_names(i)));
        end
